...
const Create: React.FC = () => {
    ...
    if (canProceed) {
        if(activeStep === 1){
            if(formData.title.trim().length > 0){
                    ...
                    toast({
                        title: "Please fill ",
                        description: "Please fill in a title",
                        status: "error",
                    });
            }
        }else{
  ...
  return (
    <div
      className="w-full mt-32 max-w-4xl mx-auto min-h-96 border border-dashed bg-white  border-neutral-200 dark:border-neutral-800 rounded-lg p-6"
      style={{
        backgroundColor: colorMode === "dark" ? "black" : "",
        borderColor:
          colorMode === "dark"
            ? "rgb(38 38 38 / var(--tw-border-opacity, 1))"
            : "",
      }}
    >
      ...
      <div className="flex justify-between mt-4">
        ...
        {activeStep < steps.length - 1 && (
          <Button onClick={handleNext} isDisabled={!canProceed} bg={buttonBg}>
            Next
          </Button>
        )}
      </div>

...
import { Box, IconButton, Text, useColorMode, useDisclosure } from "@chakra-ui/react";
...
const Suggestions = ({ videoData }: { videoData: {
  _id: string;
  title: string;
  thumbnail: string;
  owner: {
  ...
  return (
    <Box className=" p-2 rounded-lg flex gap-2 justify-between relative  transition-all duration-300" _hover={{bg: colorMode === 'dark' ? '#202020' : '#f0f0f0'}}>
      ...
      <Menu>
              ...
              <MenuList textColor={textColor} bg={'transparent'}border={'0px solid #202020'} backdropFilter="blur(20px)">
                ...
                <MenuItem bg={'transparent'} _hover={{backdropFilter:'blur(40px)'}} >Report</MenuItem>
              </MenuList>
            </Menu>
      </Box>
      <SaveToPlaylistModal isOpen={isOpen} onClose={onClose} videoId={videoData._id} />
    </Box>
  );
};

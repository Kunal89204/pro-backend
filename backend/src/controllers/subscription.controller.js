...
const toggleSubscription = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (existingSubscription) {
      const [deletedSubscription, updatedChannel] = await Promise.all([
        Subscription.findByIdAndDelete(existingSubscription._id, { session }),
      ...
      if (!deletedSubscription || !updatedChannel) {
        await session.abortTransaction();
        session.endSession();
    ...
    if (!newSubscription || !updatedChannel) {
      await session.abortTransaction();
      session.endSession();
      return res.status(500).json({
        success: false,
    ...
    return res.status(500).json({
      success: false,
      message: "Internal Server Error during subscription toggle",
    });
  }
});
